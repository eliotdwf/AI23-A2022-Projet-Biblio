...
#connexion a la BD
conn = psycopg2.connect("dbname='tgarcher_postgres_db' host='garcher.dev' port='5432' user='tgarcher' password='7&cJfhgRMhauF2*GjtbJ' options='-c search_path=projet_biblio'")
cur = conn.cursor()
...
def estAdherent(login, pwd):
    requete = "Select 1 from Adherent where login = '%s' and mdp = '%s'" % (login, pwd)
    cur.execute(requete)
    return cur.fetchone()
